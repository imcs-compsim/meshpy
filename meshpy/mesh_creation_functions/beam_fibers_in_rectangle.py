...
def create_fibers_in_rectangle(mesh, beam_object, material, length, width,
        angle, fiber_distance, fiber_element_length):
    """
    ...
    for n in range(fiber_n + 1):

        # Get the start and end point of the line.
        start, end = _intersect_line_with_rectangle(
            length, width,
            (fiber_offset + fiber_start_point + n * fiber_normal
                * fiber_distance),
            fiber_direction)
        ...
        # Create the beams if the length is not too small.
